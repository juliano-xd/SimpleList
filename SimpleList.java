...
public class SimpleList<T> implements Iterable<T> {
    ...
    public ListIterator<T> iterator() {
        return new ListIterator<T>() {
            ...
            @Override
            public T next() {
                return SimpleList.this.get(this.pointer++);
            }
            ...
            public int nextIndex() {
                return Math.min(this.pointer + 1, SimpleList.this.size());
            }

            @Override
            public int previousIndex() {
                return Math.max(this.pointer - 1, 0);
            }

            @Override
            public void remove() {
                SimpleList.this.remove(this.pointer);
            }

            @Override
            public void set(T value) {
                SimpleList.this.set(this.pointer, value);
            }

            @Override
            public void add(T value) {
                SimpleList.this.add(value);
            }
        };
    }
}